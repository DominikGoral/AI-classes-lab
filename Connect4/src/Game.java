...
public class Game {
    ...
    public void run() {
        ...
        while(!(Board.isFilled(grid.getBoard()) || (Board.boardCheckForWin(grid.getBoard(), p1) == 20 || Board.boardCheckForWin(grid.getBoard(), p2) == 20))) {
            if(playerOneMove) {
                ...
                Move actualMove = Move.findBestMove(grid.getBoard(), p1, p2);
                Board.dropToken(grid.getBoard(), actualMove.row, actualMove.col, p2.getSign());
                Board.printBoard(grid.getBoard());
